...
function Sidebar({ user, closeToggle }) {
  ...
  return (
    <div className="flex flex-col justify-between bg-white h-full overflow-y-scroll min-w-210 hide-scrollbar duration-75 ">
      ...
      {user && (
        <Link
          ...
          onClick={handleCloseSidebar}
        >
          <img src={user.image} alt="profile img" className="w-10 h-10 rounded-full" />
          <p>{user.userName}</p>
        </Link>
      )}
    </div>
